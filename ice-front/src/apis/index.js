...
const FILTER_ARTICLE_URL = () => `${API_DOMAIN}/v1/article/list/filtered`;
...
export const filterArticleRequest = async (requestBody) => {
  //asyns를 통해 비동기 함수 정의
  const result = await axios
    .post(FILTER_ARTICLE_URL(), requestBody) //await은 요청의 응답이 돌아올 떄 까지 함수 실행을 멈추는 역할 한다(asyns함수 안에서만 사용가능)
    .then((response) => {
      const responseBody = response.data; // responseBody의 key값들이: code, message, articlelist
      return responseBody;
    })
