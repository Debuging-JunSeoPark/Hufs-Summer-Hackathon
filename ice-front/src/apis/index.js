...
<<<<<<< HEAD
const MATCH_UP_URL = () => `${API_DOMAIN}/v1/article/list/filtered`;
...
export const matchUpRequest = async (requestBody) => {
    
    const result = await axios.post(MATCH_UP_URL(), requestBody) //await은 요청의 응답이 돌아올 떄 까지 함수 실행을 멈추는 역할 한다(asyns함수 안에서만 사용가능)
    .then(response => {
